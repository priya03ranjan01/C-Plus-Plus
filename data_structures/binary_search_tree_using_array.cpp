...
void create_bst(
    int a[], int index,
    int value);  // For creating and inserting element in the Binary Search Tree
void print_decreasing(
    int a[],
    int index);  // For displaying the elements of BST in decreasing order
...
int main() {
    ...
    cout << "Want to DELETE an element? Enter 'Y' or 'y' if Yes; or else any "
            "other key."
         << endl;
    cin >> ch;
    while (ch == 'y' || ch == 'Y')  // To keep on deleting till the user wants
    {
        ...
        cout << "Enter 'Y' or 'y' to delete more elements; or else any other "
                "key."
             << endl;
        cin >> ch;
    }
    ...
    cout << "Want to SEARCH an element? Enter 'Y' or 'y' if Yes; or else any "
            "other key."
         << endl;
    cin >> ch;
    while (ch == 'y' || ch == 'Y')  // To keep on searching till the user wants
    {
        ...
        cout << "Enter 'Y' or 'y' to search more elements; or else any other "
                "key."
             << endl;
        cin >> ch;
    };
...
void create_bst(int a[], int index, int value) {
    if (index >= SIZE) {
        ...
        exit(0);  // Terminate the program if there is not enough space in the
                  // array
    } else {
        if (a[index] == -1)
            a[index] = value;
            ...
            if (value < a[index])
                create_bst(a, index * 2,
                           value);  // For specifying index of left child
            else
                create_bst(a, (index * 2) + 1,
                           value);  // For specifying index of right child
        }
    }
}
...
void delete_element(int a[], int key) {
    // Searching performed initially to get the position of the specified
    // element
    int c = search_element(a, key);

    for (int i = 1; i < SIZE; i++) {
        if (a[i] == key) {
            //-2 is assigned to enable the print_decreasing function skip it
            // during printing
            a[i] = -2;
        }
    }
...
/*LOGIC: Printing the elements starting from the right most child to the left
 * most child for decreasing order*/
void print_decreasing(int a[], int index) {
    if (a[index] ==
        -2)  // Printing the elements after skipping the one to be
             // deleted (Related to logic in delete_element function)
    {
